...
describe Ezid::Record do
  describe '.delete' do
    ...
    it 'should succesfully delete an ark' do
      result = @minted_id.delete
      result.should be_kind_of Ezid::ServerResponse
      result.should_not be_errored
      @minted_id.delete.should be_errored
    end
    it 'should succesfully delete a doi' do
      result = @minted_doi.delete
      result.should be_kind_of Ezid::ServerResponse
      result.should_not be_errored
      @minted_doi.delete.should be_errored
    end
  end
  describe '.[]=' do
    before(:each) do
  ...
  describe '.save' do
    ...
    it 'should update all changed metadata' do
      @record.save
      new_record = @session.get(@record.identifier)
      new_record['Test'].should == 'blabla'
    end
  ...
  describe '.persisted?' do
    before(:each) do
      @session = Ezid::ApiSession.new
      @record = Ezid::Record.new(@session, @session.build_identifier('blabla'),
                                 { 'Test' => 1, 'Test2' => 3 })
    end
    after(:each) do
      @record.delete
    ...
    it 'should return true if an attribute is updated with an existing, persited value' do
      Ezid::ApiSession.any_instance.should_receive(:call_api).at_least(1).times
        .and_return(double('responseMock', :errored? => false))
      @record.save
      @record['Test'] = 1
